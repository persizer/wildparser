...
def find_reachable_categories():
    ...
    def process_category(category):
        # a for loop is used here to avoid skipping categories since the API often returns a blank page
        for _ in range(3):
...
def search_categories(category):
    ...
    try:
        ...
        # the print statement above always returns the error that occurs when the response is empty.
        # if needed, it can be uncommented, 
        # but i chose to comment it to avoid cluttering the terminal 
        # because the empty categories will be iterated over regardless.
...
if __name__ == '__main__':
    ...
    start_parsing()
    while len(failed_category_list) >= 50: # account for API always returning a blank page for some categories
...
""".center(100, "≡") + "\n")
    ...
    print(f"Categories that could not be analyzed due to errors:".center(100, "≡") + f"\n{failed_category_list}")
